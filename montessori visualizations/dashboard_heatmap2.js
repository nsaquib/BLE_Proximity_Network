var heatConfig = {
	timeline:{
		...
		height:400,
		margin:20,
		xScale:d3.scale.linear().domain([0,1100]).range([20,800]),
		barWidth:10,
		sliderWidth:20
	}
}
...
function timelineControlStop() {
	...
	$("#timeline-controls .stop").hide()
	clearInterval(heatConfig.timer)
}

function initTimeLine(){
	var height = heatConfig.timeline.height-120
	var width = heatConfig.timeline.width-110
	var margin =  heatConfig.timeline.margin
	
	var timeline = d3.select("#timeline").append("svg").attr("width",width)
	var xScale = heatConfig.timeline.xScale
...
function updateHandleLocations() {
	...
	var slider = d3.select("#timeline .slider")
	var startX = parseFloat(slider.attr("x")) - heatConfig.timeline.barWidth
...
function animate(data) {
	...
	$("#timeline-controls .play").click(function() {
		...
		var animationAtFrame = Math.floor(heatConfig.timeline.xScale.invert(leftHandlePosition()))
		var sliderRange = 40
		heatConfig.timer = setInterval(function(){
