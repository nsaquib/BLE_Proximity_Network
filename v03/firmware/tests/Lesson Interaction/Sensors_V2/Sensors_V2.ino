...
void loopHost() {
  ...
  //printf("%d:%d:%d:%d - Time Sent", hour, minute, second, ms);
  Serial.println(timer.hour);
  Serial.println(timer.minute);
  Serial.println(timer.second);
  Serial.println(timer.ms);
  
  int addrSwitch = hostTimer % 2;
  
  // Start GZZL stack from wake cycle
  RFduinoGZLL.hostBaseAddress = hostBaseAddresses[deviceID + addrSwitch];
  //Serial.println(hostBaseAddresses[deviceID + addrSwitch]);
  RFduinoGZLL.begin(HOST);
...
/*int setTime() {
  ...
  for (i = Serial.available(); i > 0; i--) {
    digit = (char) Serial.read();
    result += pow(10, i - 1) * (digit - 48);
  }
  return result;
}*/
