 ...
 * Created by Dwyane George, November 17, 2015.
...
void changeDeviceRole() {
  int randomNumber = (int) random(MAX_DEVICES);
  if (randomNumber < EXPECTED_HOSTS) {
    setupHost();
  } else {
    setupDevice();
  }
...
void sleepUntilStartTime() {
  sleepDevice(timer.getTimeUntilStartTime(START_HOUR, START_MINUTE));
}
